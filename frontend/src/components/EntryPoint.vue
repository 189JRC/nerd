            ...
            <div v-if="element.label == 'PERSON' || element.label == 'IDENTIFIED PERSON'"
              ...
              <div class="dark:bg-gray-200 rounded-xl ">
                <draggable @change="update_network_data(element.name, 'location', $event)"
                  v-if="element.label == 'PERSON' || element.label == 'IDENTIFIED PERSON'" class="list-group"
                  :list="element.relevant_details.locations" group="people" itemKey="name">
                  <template #item="{ element, index }">
                    <div
              ...
              <div class="dark:bg-gray-200 rounded-xl ">
                <draggable @change="update_network_data(element.name, 'relevant_details', $event)"
                  v-if="element.label == 'PERSON' || element.label == 'IDENTIFIED PERSON'" class="list-group"
                  :list="element.relevant_details.details" group="people" itemKey="name">
                  <template #item="{ element, index }">
                    <div
                      class="mt-2 bg-blue-300 text-black px-4 py-2 rounded-md shadow-md hover:bg-blue-700 cursor-pointer">
                      {{ element.name }} </div>
...
export default {
  ...
  data() {
    return {
      ...
      chunk_index: null,
      show_popup: false,
      data_entity_selected_for_relevance: null,
      search_target: null,
  ...
  updated() {
    // when dom is updated by text highlighting/ unhighlighting event listeners need to be removed/applied
  ...
  methods: {
    ...
    format_vector_search_responses_for_display() {
      ...
      for (let index = 0; index < 10; index++) {
        ...
        //need to make it into html as the v-html binding does not take template syntax
        var chunk_html =
    ...
    build_network() {
      // TODO:1: This really needs to be a child component!
      console.log("build network")
      const nodes = []
      const edges = []
      //map is like a js object, but maintains the order of its entries
      //it also has methods such as .has to check for entries which js objects dont
      const node_map = new Map()
      //function to add nodes if they don't already exist
    ...
    highlightText() {
      ...
      //TODO:2: find a better way to do this. it is hacky and has required css.
      //CSS selectors where used to avoid the long winded tailwind class attributes
    ...
    mark_as_significant(type) {
      ...
      if (new_significant_object && type == 'entity') {
        this.investigation.entities.push(new_significant_object)
    ...
    make_insignificant() {
      ...
      // TODO:1: Need to gether bunch these terms up and send them to the back end to make search queries/filters out of.
